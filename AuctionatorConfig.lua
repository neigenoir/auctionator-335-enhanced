...
function Atr_InitOptionsPanels()
	...
	Atr_SetupExperimentalOptionsFrame();
...
function Atr_SetupExperimentalOptionsFrame ()

	UIDropDownMenu_Initialize(Atr_cacheThresholdDD, Atr_cacheThresholdDD_Initialize);
	UIDropDownMenu_SetSelectedValue(Atr_cacheThresholdDD, AUCTIONATOR_CACHE_THRESHOLD);
end

-----------------------------------------

function Atr_ExperimentalOptionsFrame_Save()

	local origValues = zc.msg_str (AUCTIONATOR_CACHE_THRESHOLD);

	AUCTIONATOR_CACHE_THRESHOLD = UIDropDownMenu_GetSelectedValue(Atr_cacheThresholdDD);

	local newValues = zc.msg_str (AUCTIONATOR_CACHE_THRESHOLD);

	if (origValues ~= newValues) then
		zc.msg_atr (ZT("experimental options saved"));
	end
...
function Atr_cacheThresholdDD_Initialize()

	local info = UIDropDownMenu_CreateInfo();
	
	Atr_AddMenuPick (info, ZT("No caching"),						0, Atr_cacheThresholdDD_OnClick);
	Atr_AddMenuPick (info, ZT("10 seconds"),					10, Atr_cacheThresholdDD_OnClick);
	Atr_AddMenuPick (info, ZT("30 seconds"),					30, Atr_cacheThresholdDD_OnClick);
	Atr_AddMenuPick (info, ZT("1 minute"),						60, Atr_cacheThresholdDD_OnClick);
	Atr_AddMenuPick (info, ZT("5 minutes"),					300, Atr_cacheThresholdDD_OnClick);
	Atr_AddMenuPick (info, ZT("30 minutes"),					1800, Atr_cacheThresholdDD_OnClick);
	Atr_AddMenuPick (info, ZT("1 hour"),						3600, Atr_cacheThresholdDD_OnClick);
	Atr_AddMenuPick (info, ZT("Always (until reload)"),		999999, Atr_cacheThresholdDD_OnClick);

end

-----------------------------------------

function Atr_cacheThresholdDD_OnClick(self)
	UIDropDownMenu_SetSelectedValue(self.owner, self.value);
end

-----------------------------------------

function Atr_cacheThresholdDD_showTip(self)

	GameTooltip:SetOwner (self, "ANCHOR_RIGHT");
	GameTooltip:SetText (ZT("Cache search results"));
	GameTooltip:AddLine (ZT("How long to keep search results in memory before searching again"), 1.0, 1.0, 1.0, 1);
	GameTooltip:Show();
