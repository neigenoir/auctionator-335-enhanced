...
function AtrSearch:Init (searchText, exact, rescanThreshold, callback)
	...
	-- Cache logic works for both exact and partial search
...
function AtrSearch:AnalyzeResultsPage()
	...
	if (totalAuctions > 0) then
		...
		Atr_SetMessage (string.format (ZT("Scanning auctions: page %d/%d"), self.current_page, totalPages));
	end
	...
	if (numBatchAuctions > 0) then
		...
		for x = 1, numBatchAuctions do
			...
			if (exactMatch or not self.exact) then
				...
				local timeLeft = GetAuctionItemTimeLeft("list", x);
				scn:AddScanItem (name, count, buyoutPrice, owner, 1, curpage, bidAmount, minBid, minIncrement, timeLeft, highBidder);
...
function AtrScan:AddScanItem (name, stackSize, buyoutPrice, owner, numAuctions, curpage, bidAmount, minBid, minIncrement, timeLeft, highBidder)
	...
	for i = 1, numAuctions do
		-- Only store what we need for grouping and display
		sd["stackSize"]		= stackSize;      -- For grouping same-size stacks
		sd["buyoutPrice"]	= buyoutPrice;    -- For grouping same-price auctions
		sd["owner"]			= owner;          -- For "yours" flag
		sd["highBidder"]	= highBidder;     -- For "yours" flag
		sd["pagenum"]		= curpage;        -- For page tracking
		--sd["nextBid"]		= bidAmount and bidAmount > 0 and (bidAmount + minIncrement) or minBid;  -- For display
		--sd["nextBid"] 		= (bidAmount and bidAmount > 0 and (bidAmount + minIncrement + 1)) or (minBid and minBid + 1) or 0
		sd["nextBid"] = (bidAmount and bidAmount > 0 and (bidAmount + minIncrement))
             or (minBid and minBid)
             or 0
		sd["hasActiveBids"]	= bidAmount and bidAmount > 0;  -- For coloring
		sd["timeLeft"]		= timeLeft;
...
function AtrSearch:Continue()
	...
	if (self.items[self.searchText] and self.items[self.searchText].whenScanned > 0) then
		self.processing_state = KM_NULL_STATE;
		-- Call the same completion function as normal search
...
--	zc.msg_dev (queryString.."  page:"..self.current_page);
...
--			ownerCode = sd.owner;
		...
		if (conddata[key]) then
			...
			-- Take the minimum nextBid from the group
			if (sd.nextBid) then
				conddata[key].nextBid = conddata[key].nextBid and zc.Min(conddata[key].nextBid, sd.nextBid) or sd.nextBid;
			end
			-- If any auction in group has active bids, mark the group
			conddata[key].hasActiveBids = conddata[key].hasActiveBids or sd.hasActiveBids;
		else
			local data = {};
...
function AtrScan:PrepAutoSell (currentPane, stackSize, numToCreate)

	currentPane[ZF("6ONIy-FFx-69S")] = true;
	if (stackSize == 1 and currentPane[ZF("_OFFyN68EyCT-")] > 1) then
		if (self[ZF("HOG4IOLyCHAF-NIHM")] + numToCreate > kPrefBaseSize) then
			currentPane[ZF("6ONIy-FFx-69S")] = nil;
		end
	end
end
...
function Atr_FullScanAnalyze()
	...
	zc.msg_dev ("FULL SCAN:"..numBatchAuctions.." out of  "..totalAuctions)
	...
	for name,prices in pairs (lowprices) do
		...
		if (newprice > 0) then
			...
			if (qx < AUCTIONATOR_SCAN_MINLEVEL and gAtr_ScanDB[name]) then
				...
				zc.msg_dev ("removed: |cffbbbbbb", name, "   ("..qx..")");
			end
